...
class MailTaskLogsController < ApplicationController 
  ...
  def set_params_and_find(params)
    ...
    params[:status] = states.map &:to_i
    params[:selected_status] = params[:status]
    # params[:status] = params[:journal_entry][:state] if params[:journal_entry] && params[:journal_entry][:state] 
    puts "selected_state: #{params[:selected_state].inspect} state: #{params[:state].inspect}"
    @state = MailTask.states.values if params[:status].blank?
    @start_date = 2.months.ago
    @stop_date = DateTime.now.end_of_day
    @follow_up = params[:follow_up].to_i

    tasks_relation = MailTask.with_status(@state).between(@start_date, @stop_date)
    if @follow_up > -1
      tasks_relation = tasks_relation.where(follow_up: @follow_up)
    end
    @task_count = tasks_relation.count
    @tasks = task_relation.all(:order => 'created_at asc', :include => :journal)
