...
#    q_type = questiontype # + n.to_s      
  ...
  def question_item_form(question_item, args)
    ...
    optno -= 1 if question_item.split("|").last == "text"  # make yes/no/text a rating2
      ...
      when "options" then          # select options
      ...
      when "Text" then text_area(cell, "QuestionText", {:cols => 18, :rows => 4})
      when "Information" then text_area(cell, "Information", {:cols => 50, :rows =>4})
      when "ListItem" then create_textfields(1, cell, "", "ListItem")
      when "ListItemComment" then create_textfields(1, cell, "", "ListItem") + create_textareas(1, cell, "", "ListItemBox")
      when "ListItemx2" then create_textfields(2, cell, "", "ListItem")
      when "TextBox" then create_textareas(1, cell, "", "TextBox") #text_area(cell, "QuestionText", {:cols => 18, :rows => 4}) #
        #    when "SelectList": create_selectoptions(2, cell, "", "selectoption") # "SelectList not yet implemented"
      when "CheckboxAccess" then create_checkbox(1, cell, ["Ingen"], "Checkbox")
      when "Checkbox" then create_checkbox(1, cell, ["Tekst"], "Checkbox")
      when "Placeholder" then hidden_field(cell, "placeholder") # create_textfields(1, cell, "", "placeholder")  # create empty space
      when "" then  # flash, change_back_form# do nothing. fix. Same as pressing change
  ...
  def rating_choices(type)
    ...
    options = case type
    when "Rating_0" then ratings_0.to_a
    when "Rating_1" then ratings_1.to_a
    when /Rating(\d)_(\d)/ then eval("choices#{$1}_#{$2}").to_a
  ...
  def description_form(type, row, col)
    ...
    # when "Description1_7": description1_7
      when "Description_2" then description2 
      when "Description_3" then description3 
      when "Description_4" then description4 
      when "Description_5" then description5 
      when "Description_6" then description6 
      when "Description_7" then description7
