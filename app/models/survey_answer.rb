...
class SurveyAnswer < ActiveRecord::Base
  ...
  def save_final(params)
    ...
    csv_task = CsvSurveyAnswerTask.new :survey_answer => self, :status => 'To do'
        ...
        if cell =~ /q(\d+)_(\d+)_(\d+)/   # match col, row
          ...
          if answer_cell = an_answer.exists?(a_cell[:row], a_cell[:col]) # update
            if the_valid_values[q].nil?
              ...
              logger.info "valid_values[q][cell] #{q.inspect} not found. cell: #{cell.inspect}. a_cell: #{a_cell.inspect}  vv: #{q.inspect}"
              puts "valid_values[q] #{q.inspect} not found. cell: #{cell.inspect}"
            end
  ...
  def save_csv_survey_answer
    ...
    if !self.journal
      logger.info "No journal: #{self.inspect}"
      return
    end
