...
class Journal < ActiveRecord::Base #< Group
  ...
  validates_format_of :parent_email, :with => /^(|(([A-Za-z0-9]+_+)|([A-Za-z0-9]+\-+)|([A-Za-z0-9]+\.+)|([A-Za-z0-9]+\++))*[A-Za-z0-9]+@((\w+\-+)|(\w+\.))*\w{1,63}\.[a-zA-Z]{2,6})$/i, :multiline => true
  ...
  def self.header_info
    ["ssghafd","ssghnavn","safdnavn","pid","projekt","pkoen","palder","pnation","besvarelsesdato","pfoedt"]
  end
