class Variable < ActiveRecord::Base
  ...
  @@survey_variables = {
    ...
    9 => "asq1;asq2;asq3;asq4;asq5;asq6;asq7;asq8;asq9;asq10;asq11;asq12;asq13;asq14;asq15;asq16;asq17;asq18;asq19;asq20;asq21;asq22;asq23;asq24;asq25;asq26;asq27;asq28;asq29;asq30;asq31;asq32;asq33;asq34;asq35"
  }
  ...
  def self.create_by_cell(text, cell)
    return "" if cell.nil?
    dtype = 
    if cell.items.include?("listitem")
      "string" 
    elsif cell.items.include?("radio") || cell.items.include?("checkbox")
      "numeric"
    else "unknown"
    end

    # t = text.gsub("listitem::::", "").gsub("::####radio","").gsub("checkbox::", "")
    Variable.new(:var => "asq#{cell.answer_item}", :text => text, :item => cell.answer_item, :row => cell.row, :col => cell.col, 
      :datatype => dtype, :question_id => cell.question_id)
  end

  def self.create_by_survey(survey)  # specific for ASQ-SE
    # assume first question has variables (valid for asq)
    q = survey.questions.first
    rows = q.question_cells.group_by {|qc| qc.row}
    first_row = rows[1]
    desc_item1 = first_row[1]
    desc_item2 = first_row[2]

    header = []<< first_row.map {|t| t.items.gsub("listitem::::", "").gsub("::####radio","").gsub("checkbox::", "")}
    
    description = rows[1]
    vars = rows.to_a[1..-1].map do |row|
      rownum = row.first
      cells = row.last
      text_cell = cells.first
      text = text_cell.items.gsub("listitem::::", "")
      rating = cells[1]
      checkbox = cells[2] 
      variable1 = Variable.create_by_cell(text, rating)
      variable2 = Variable.create_by_cell(text, checkbox)
      [variable1, variable2]
    end
    header << vars
  end
