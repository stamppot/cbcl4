			...
			if(has_no_text)     	# listitem without predefined text
				if(disabled)      	# show answer value
			    	...
			    	when /create|edit/ then
		            	field << "<textarea id='#{c_id}' class='textfield' name='#{question_no}[#{cell_id(no)}]' type='text' maxlength='1900' rows='1'>#{value}</textarea>"
        		    	newform << field
          			end
				end
...
class SelectOption < QuestionCell
	...
	def to_answer(options = {})
		...
		newform << "<span class='answer_value'>#{text_value}</span>"
		span_item(newform.join, "#{outer_span}".rstrip)
	end
		  ...
		  case item.qtype
			...
			when "textbox" then 
				...
				if (listitem_without_predefined_text)
					answer_val = self.value.to_s.blank? ? "" : "<div id='#{c_id}' class='comment answer_value'><span>#{CGI.unescape(self.value.to_s)}</span></div>"
		...
		self.question_items.each do |item|
		  ...
		  case item.qtype
			...
			when "textbox" then 
				...
				if (listitem_without_predefined_text)
					input_or_answer = answer ?
					  ...
					  "<textarea id='#{c_id}' name='#{question_no}[#{c_id}]' maxlength='1900' class='comment' cols='#{tcols}' rows='#{trows}' #{disabled ? ' disabled style="display:none;"' : ''}>#{self.value}</textarea>"
			...
			when "listitem" then 
				...
				if (listitem_without_predefined_text)
					span_item(((answer_item_set && self.col > 2) ? "" : answer_item) + 
					"<textarea id='#{c_id}' name='#{question_no}[#{c_id}]' class='textfield' type='text' maxlength='1900' rows='1' value='#{item.value}'>#{self.value}</textarea>", "listitemfield #{span} #{target}")
...
class Rating < QuestionCell
	...
	def to_answer(options = {})
		if choice
			...
			if self.span && datatype == :numeric
				span_item(text, "span-#{self.span} answer_value")
			elsif question.columns == 2 && datatype == :numeric
