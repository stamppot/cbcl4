...
q2_cells = [
Information.new({:question => q2, :col => 1, :row => 1, :answer_item => nil, :items => "information::::Her er en liste over egenskaber, som kan være til stede i større eller mindre grad. Sæt en ring rundt om 2, hvis beskrivelsen passer godt eller ofte. Sæt ring rundt om 1, hvis beskrivelsen passer til en vis grad eller nogen gange. Hvis beskrivelsen ikke passer på barnet, sæt ring om 0. Svar venligst så godt du kan på alle spørgsmålene. ", :preferences => "", :prop_mask => 0}),
ListItem.new({:question => q2, :col => 1, :row => 2, :answer_item => nil, :items => "textbox::::Hvis aftalt andet end 7 dage, angiv hvor mange dage, det passer på dig", :preferences => "", :prop_mask => 0}),
